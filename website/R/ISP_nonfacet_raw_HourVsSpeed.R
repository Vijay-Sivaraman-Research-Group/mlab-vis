...
#
# Description:
#   This script creates raw speed plots as a function of hour in a particular year,month for all input ISPs
#
# Instructions:
...
#
# Output:
#   1. Raw speed plot as a function of hour for a particular year (user input), aggregated by mean
#   2. Raw speed plot as a function of hour for a particular year (user input), aggregated by median
#
##########################################################################################################################################################
...
Average_value_median <- aggregate(download_speed ~ year+ISP+hour, house_total_filtered, median)
Average_value_median$hour <- as.POSIXct(strptime(Average_value_median$hour, format = "%H"), format = "%y/%m/%d %H:%M:%S")



graph_name = paste("plot__",Average_value_median$year,"_", month, "__","raw__hour_speed_ISPs__", ISPs_list,"___", sep="")
SpeedHour_Plot <- ggplot(Average_value_median, aes(x = hour, y = download_speed, color = ISP, shape = ISP, color=supp)) +
  geom_point(size=2.5)+
  geom_line(size=1)+
  ...
  ggtitle(graph_name)+
  theme_bw()
savefileName =paste("plot__",Average_value_median$year,"_", month, "__","raw__hour_speed_ISPs__", ISPs_list,"___",".png", sep="")
...
# #--------------------------------- Colour by ISP (mean)-----------------------------------#
# 
# Average_value_mean <- aggregate(download_speed ~ year+ISP+hour, house_total_filtered, mean)
# Average_value_mean$hour <- as.POSIXct(strptime(Average_value_mean$hour, format = "%H"), format = "%y/%m/%d %H:%M:%S")
# 
# graph_name = paste(Average_value_mean$year[1]," Raw speed by hour plot (mean) ")
# SpeedHour_Plot <- ggplot(Average_value_mean, aes(x = hour, y = download_speed, color = ISP, shape = ISP, color=supp)) +
#   geom_point(size=2.5)+
#   geom_line(size=1)+
#   ylab("Download speed (mbps)") +
...
#   ggtitle(graph_name)+
#   theme_bw()
# savefileName = paste(Average_value_mean$year[1],"_",Average_value_mean$month[1],"_","raw_HourVsSpeed_ISP_mean_nonfacet.png", sep="")
# ggsave(filename=savefileName, plot=SpeedHour_Plot, path = dest_path, width = 10, height = 6)
