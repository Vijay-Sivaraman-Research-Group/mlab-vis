...
#
# Description:
#   This script creates normalised speed plots as a function of month in a particular year for all input ISPs
#
# Instructions:
...
#
# Output:
#   1. Nomalised speed plot as a function of month for a particular year (user input), aggregated by median
#
##########################################################################################################################################################
...
Average_value_median <- aggregate(download_speed_normalised_month ~ ISP+month+year, house_total_filtered, median)
...
graph_name = paste("plot__",Average_value_median$year, "__","norm__month_speed_ISPs__", ISPs_list,"___", sep="")
SpeedMonth_Plot <- ggplot(Average_value_median, aes(x = MonthYear, y = download_speed_normalised_month, color = ISP, shape = ISP, color=supp)) +
  geom_point(size=2.5)+
  geom_line(size=1)+
  ...
  ggtitle(graph_name)+
  theme_bw()
savefileName = paste("plot__",Average_value_median$year, "__","norm__month_speed_ISPs__", ISPs_list,"___",".png", sep="")
...
# 
# 
# Average_value_mean <- aggregate(download_speed_normalised_month ~ ISP+month+year, house_total_filtered, mean)
...
# 
# graph_name = paste(Average_value_mean$year[1]," Normalised speed by month plot (mean) ")
# SpeedMonth_Plot <- ggplot(Average_value_mean, aes(x = MonthYear, y = download_speed_normalised_month, color = ISP, shape = ISP, color=supp)) +
#   geom_point(size=2.5)+
#   geom_line(size=1)+
#   scale_x_datetime(labels = date_format("%b %Y", tz="Australia/Sydney"))+
#   ylab("Normalised download speed") +
...
#   ggtitle(graph_name)+
#   theme_bw()
# savefileName = paste(Average_value_mean$year[1],"_","norm_MonthVsSpeed_ISP_mean_nonfacet.png", sep="")
# ggsave(filename=savefileName, plot=SpeedMonth_Plot, path = dest_path, width = 10, height = 6)
# 
